...
public class EntitySearcher6 extends EntitySearcher {
	...
	public ArrayList<EntityProposals> searchEntity(
		...
		if(printMatchingDetails) System.out.println("EntitySearcher6: search '"+entityphrase+"[orig="+originalentityphrase+"]'");
		...
		if(tokens.length>=2){ //to prevent "rostral tubule" from entering the subsequent process 
			...
			if(!shortened.matches(".*?\\b("+Dictionary.spatialtermptn+")$")){
				...
				if(shortentities!=null){
					if(printMatchingDetails) System.out.println("search for entity '"+shortened+"' found match, forming proposals...");
						...
						if(sentity.isOntologized() && entityls!=null){
							for(FormalConcept fc: entityls){
								...
								if(SearchMain.elk!=null && SearchMain.elk.isSubclassOfWithPart(entityl.getClassIRI(), sentity.getClassIRI())){
									if(printMatchingDetails) System.out.println("'"+entityl.getLabel() +"' and '"+sentity.getLabel() + "' are related, increase confidence score");
									found = true;
									...
									if(printMatchingDetails) System.out.println("add a proposal with anatomical entity:"+centity.toString());
								}
							}
						}
					}*/
					if(found){
						...
						if(printMatchingDetails) System.out.println("EntitySearcher6 returns:");
					...
					if(printMatchingDetails) System.out.println("entity and entity locator (if exists) are not related");
					...
					for(FormalConcept sentityfc: shortentities){				
						...
						if(elocatorphrase.length()>0){
							for(FormalConcept fc: entityls){
								...
								if(printMatchingDetails) System.out.println("add a proposal without anatomical entity:"+centity.toString());*/
							...
							if(printMatchingDetails) System.out.println("add a proposal without anatomical entity:"+sentity.toString());*/
				...
				if(sentities!=null){
					if(printMatchingDetails) System.out.println("search for entity '"+shortened+"\\b.*' found match, forming proposals...");
					//construct anatomicalentity
						...
						if(sentity.isOntologized() && entityls!=null){
							for(FormalConcept fc: entityls){
								...
								if(SearchMain.elk!=null && SearchMain.elk.isSubclassOfWithPart(entityl.getClassIRI(), sentity.getClassIRI())){
									...
									if(printMatchingDetails) System.out.println("add the other proposal without anatomical entity:"+centity.toString());		
								}
							}
						}
					}*/
		...
		if(printMatchingDetails) System.out.println("EntitySearch6 found no match");
		return null;
	}
