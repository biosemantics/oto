...
public class EntitySearcherOriginal extends EntitySearcher {
	...
	public ArrayList<EntityProposals> searchEntity(String entityphrase, String elocatorphrase, String originalentityphrase, String prep, float discount){
		////if(printMatchingDetails) System.out.println("search entity: "+entityphrase);
		//create and maintain a cache for entity search?: yes, created in EntityParser
		if(printMatchingDetails) System.out.println("EntitySearcherOriginal: search '"+entityphrase+"[orig="+originalentityphrase+"]'");
		...
		for(String aentityphrase: entityphrases){
			for(String aelocatorphrase: elocatorphrases){
				...
				if(printMatchingDetails) System.out.println("EntitySearcherOriginal calls EntitySearcher1");
		...
		//logging
		if(entities!=null){	
			if(printMatchingDetails) System.out.println("EntitySearcherOriginal completed search for '"+entityphrase+"[orig="+originalentityphrase+"]' and returns:");
	...
	if(printMatchingDetails) System.out.println("EntitySearcherOriginal: no match in ontology is found for '"+entityphrase+"','"+elocatorphrase+"', form string-based proposals...");
	...
	if(elocatorphrase.length()>0){
		...
		ep.add(centity);
		if(printMatchingDetails) System.out.println("add a proposal:"+centity.toString());
		//entities.add(ep);
		Utilities.addEntityProposals(entities, ep);
	}else{
		//EntityProposals entities = new EntityProposals();
		...
		ep.setPhrase(originalentityphrase);
		ep.add(sentity);
		if(printMatchingDetails) System.out.println("add a proposal:"+sentity.toString());
		//entities.add(ep);
		Utilities.addEntityProposals(entities, ep);
	}
