	...
	if (XHR.readyState == 4) {
		...
		if (XHR.status == 200) {
			...
			if (response.charAt(0) != 'E') {
				if (adminFlag) {
					newDiv.innerHTML = '<a href="#" onclick="updateStatus('
							+ response + ',\'' + divId
							+ '\', false)">Revoke</a>';
...
/**
 * this is for categorizing page only. Besides setTerm, it needs to set
 * reviewedTerms also, if current is not context or glossary
 * 
 * @param term
 */
...
function showTermLocations() {
	...
	$("#th_context_1").show();
	$("#th_context_2").show();
	document.getElementById("th_context_1").innerHTML = "Copy (of "
			+ getOriginalTermName(term) + ")";
	...
	for (j = 0; j < 100; j++) {
		...
		if (objs.length < 1) {
			...
			td.innerHTML = "<font class=\"font-text-style\">" + nameOfCopy
					+ "</font>";
			var td1 = document.createElement("td");
			var category = "uncategorized";

			var temp = ex.parentNode;
			while (temp.className != "categoryTable"
					&& temp.id != "availableTerms") {
			...
			td1.innerHTML = "<font class=\"font-text-style\">" + category
					+ "</font>";
			tr.appendChild(td);
			tr.appendChild(td1);
			tab1.appendChild(tr);
...
function getGlossary() {
	...
	var tab = document.getElementById("contextTable");
	tab.innerHTML = "<font class='font-text-style'>&nbsp;&nbsp;&nbsp;&nbsp;Looking up glossaries for '"
			+ term + "'...</font>";
...
function getContext() {
	...
	document.getElementById("th_context_2").innerHTML = "";
	var tab = document.getElementById("contextTable");
	tab.innerHTML = "<font class='font-text-style'>&nbsp;&nbsp;&nbsp;&nbsp;Looking up context for '"
			+ term + "'...</font>";
	...
	if (XHR) {
		...
		XHR.setRequestHeader("Content-Type",
				"application/x-www-form-urlencoded; charset=UTF-8");
		var x = 'value=' + term + "&uploadID=" + $("#UPLOADID").html();
		XHR.send(x);
	}
}

/*
 * This Ajax call is made to check if the user entered a mail id that is already
 * there in the system, during registration
 */
...
function emailCheck() {
	if (XHR.readyState == 4) {
		// only if "OK"
		if (XHR.status == 200) {
			var emailResponse = XHR.responseXML
					.getElementsByTagName("response")[0].childNodes[0].nodeValue;
			if (emailResponse != 'Available') {
				document.getElementById("emailCheck").innerHTML = "<font class=\"font-text-style\">"
						+ emailResponse + "</font>";
...
function processGlossary() {
	if (XHR.readyState == 4) {
		...
		if (XHR.status == 200) {
			...
			$(th0).attr("align", "left");
			$(th0).html("ID (of " + termName + ")");
			$(th0).attr("width", "20%");
			row.appendChild(th0);

			var th1 = document.createElement("th");
			$(th1).html("Category (of " + termName + ")");
			$(th1).attr("align", "left");
			$(th1).attr("width", "20%");
			row.appendChild(th1);

			var th2 = document.createElement("th");
			$(th2).html("Definition (of " + termName + ")");
			$(th2).attr("align", "left");
			$(th2).attr("width", "20%");
			row.appendChild(th2);
			...
			for ( var i = 0; i < glossaries.length; i++) {
				...
				if (glossaries[i].childNodes[0].childNodes[0]) {
					var category = glossaries[i].childNodes[0].childNodes[0].nodeValue;
					td.innerHTML = "<font class=\"font-text-style\">"
							+ category + "</font>";
				}

				var td1 = document.createElement("td");
				if (glossaries[i].childNodes[1].childNodes[0]) {
					td1.setAttribute("width", "10%");
					td1.innerHTML = "<font class=\"font-text-style\">"
							+ glossaries[i].childNodes[1].childNodes[0].nodeValue
							+ "</font>";
				}

				var td2 = document.createElement("td");
				if (glossaries[i].childNodes[2].childNodes[0]) {
					td2.setAttribute("width", "70%");
					td2.innerHTML = "<font class=\"font-text-style\">"
							+ glossaries[i].childNodes[2].childNodes[0].nodeValue
							+ "</font>";
				}
...
function processContext() {
	...
	if (XHR.readyState == 4) {
		...
		if (XHR.status == 200) {
			...
			$("#th_context_1").show();
			$("#th_context_2").show();
			document.getElementById("th_context_1").innerHTML = "Source (of "
					+ termName + ")";
			...
			for ( var i = 0; i < contexts.length; i++) {
				...
				/**
				 * old: file name clickable
				 */
				// td.innerHTML = "<font class=\"font-text-style\"><a href=\"#\"
				// onClick=\"showFile('"
				// + sourceFileName
				// + "')\">"
				// + sourceFileName
				// + "</a></font>";
				/**
				 * file name not clickable for now: 1st version
				 */
				td.innerHTML = "<font class=\"font-text-style\">"
						+ sourceFileName + "</font>";
...
function showFileText() {
	if (XHR.readyState == 4) {
		...
		if (XHR.status == 200) {
			...
			// fileText =
			// XHR.responseXML.getElementsByTagName("text")[0].childNodes[0].nodeValue;
