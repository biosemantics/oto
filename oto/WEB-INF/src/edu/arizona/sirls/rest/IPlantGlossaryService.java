...
import edu.arizona.sirls.rest.beans.Category;
import edu.arizona.sirls.rest.beans.iplant.TermCategory;
import edu.arizona.sirls.rest.beans.iplant.TermSynonym;
import edu.arizona.sirls.rest.beans.iplant.GlossaryDownload;
import edu.arizona.sirls.rest.db.CategoryDAO;
import edu.arizona.sirls.rest.git.GlossaryDownloadDAO;
import edu.arizona.sirls.rest.git.GlossaryTypeDAO;
import edu.arizona.sirls.rest.git.GlossaryVersionDAO;
...
public class IPlantGlossaryService {
	...
	private GlossaryDownload getLegacyGlossaryDownloadForIplant(
			edu.arizona.sirls.rest.beans.GlossaryDownload oldGlossaryDownload) {
		List<edu.arizona.sirls.rest.beans.TermCategory> oldTermCategories = oldGlossaryDownload.getTermCategories();
		List<edu.arizona.sirls.rest.beans.TermSynonym> oldTermSynonyms = oldGlossaryDownload.getTermSynonyms();
		List<TermCategory> termCategories = new LinkedList<TermCategory>();
		List<TermSynonym> termSynonyms = new LinkedList<TermSynonym>();
		for(edu.arizona.sirls.rest.beans.TermCategory oldTermCategory : oldTermCategories)
			termCategories.add(new TermCategory(oldTermCategory.getTerm(), oldTermCategory.getCategory(), oldTermCategory.isHasSyn()));
		for(edu.arizona.sirls.rest.beans.TermSynonym oldTermSynonym : oldTermSynonyms)
