...
<%@page import="edu.arizona.sirls.beans.TagBean"%>
<%@page import="edu.arizona.sirls.beans.StructureNodeBean"%>
<%@ page import="java.util.ArrayList"%>
<%@ page import="java.util.HashMap"%>
<%@ page import="edu.arizona.sirls.util.Utilities"%>
<%@ page import="edu.arizona.sirls.beans.Order"%>
<%@ page import="edu.arizona.sirls.beans.Character"%>
...
<table border="1" width="100%">
	<tr>
		<td><font class="font-text-style" id="dataset_info">Current Dataset: <b><%=dataset %></b>
		</font></td>
	</tr>
<%
 ...
 %>
	<tr>
			...
			<tr>
				...
				<table>
					<tr>
						...
						<td align="right">
						<form id="<%=baseOrder.getBaseOrderID()%>_form" name="generalForm"
							method="post"><input type="button" name="button"
							value="Save Orders"
							class="uiButton uiButtonSpecial uiButtonMedium"
							style="padding: 0px 1px 1px 1px" onclick="save_order(this)"
							onmouseover="document.getElementById('<%=baseOrder.getID()%>_savingMsg').innerHTML='&nbsp;'" />
						<input type="hidden" name="orderID"
							value="<%=baseOrder.getBaseOrderID()%>" /> <input type="hidden"
							name="baseOrderID" value="<%=baseOrder.getID()%>" /> <input
							type="hidden" id="<%=baseOrder.getID()%>_hiddenvalue"
							name="value" />&nbsp;<img src="images/green_rot.gif"
							id="<%=baseOrder.getID()%>_processingSaveImage"
							style="visibility: hidden" width="15px;" /><font
							class="savingMsg"
							onmouseover="document.getElementById('<%=baseOrder.getID()%>_savingMsg').innerHTML='&nbsp;'"
							id="<%=baseOrder.getID()%>_savingMsg" style="color: green">&nbsp;</font></form>
						</td>
					</tr>
				</table>
				</th>
							...
							if (order.savedBefore()) {
												...
												for (int i = 0; i < order_terms.size(); i++) {
													...
													if (currentPosition == lastPosition) {
														...
														out.print("<div id = '"
																+ order_terms.get(i).getName()
																+ "'"
																+ (order_terms.get(i).isBase() ? "rel='base'"
																		: "") + ">"
																+ order_terms.get(i).getName()
																+ "</div>");
													} else {
		...
		%>
		<font class="font-text-style">There is no task for you now. </font>
		<%
				}
		%>
		</div>
...
<%} else {%>
	<tr>
		<td height="480px" align="center"><font class="font-text-style" color="green" style="font-size: 15px">
		Dataset <b><%=dataset %></b> has been reviewed and finalized!  <br></br>
		You can <a href="javascript:void(0)" onclick="startDownload('<%=dataset%>')">click here</a> to download this finalized term set.  (<a href="gotoDownload.do">Download more</a>) </font><br></br>
