...
import edu.arizona.biosemantics.form.RegistrationForm;
import edu.arizona.biosemantics.util.ApplicationConstants;
import edu.arizona.biosemantics.util.EncryptDecrypt;
...
public class EditSettingsAction extends ParserAction {
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		if(checkSessionValidity(request)) {
			...
			EncryptDecrypt edc = EncryptDecrypt.getInstance();
			String oldPass = edc.encrypt(regForm.getRegPassword());
			...
			if(newPassword != null && !newPassword.equals("") 
					&& newPassword.length() < ApplicationConstants.PASSWORD_LENGTH) {
				errorFlag = true;
				message.append("The minimum password length is "+ ApplicationConstants.PASSWORD_LENGTH+" characters, ");
			}
			...
			if (errorFlag) { //generate validation error msg
				...
				try {
					...
					if (passChanged) {
						user.setPassword(edc.encrypt(newPassword));	
					}
