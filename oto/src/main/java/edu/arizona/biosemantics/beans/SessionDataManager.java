...
public class SessionDataManager implements Serializable {
	...
	public SessionDataManager(User user) {
		...
		try {
			...
			if (datasets == null) {
				datasets = Utilities.getDataSets();
			}
	...
	public ArrayList<String> getGroupNames() throws Exception {
		if (groupNames == null) {
			groupNames = utilities.getGroups(dataset, user.getUserId(), false);
		}
		return groupNames;
	}
	...
	public void refetchDecisions() throws Exception {
		decisions = utilities.getDecisions(dataset);
	}
	...
	public void refetchUserDecisions() throws IOException{
		...
		try {
			for (String groupDataset : datasets) {
				ArrayList<DecisionBean> userGroupReport = utilities.getUserSpecificReport(user, groupDataset);
