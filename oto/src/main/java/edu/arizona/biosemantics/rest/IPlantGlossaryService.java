package edu.arizona.biosemantics.rest;
...
import edu.arizona.biosemantics.rest.beans.Category;
import edu.arizona.biosemantics.rest.beans.iplant.GlossaryDownload;
import edu.arizona.biosemantics.rest.beans.iplant.TermCategory;
import edu.arizona.biosemantics.rest.beans.iplant.TermSynonym;
import edu.arizona.biosemantics.rest.db.CategoryDAO;
import edu.arizona.biosemantics.rest.git.GlossaryDownloadDAO;
import edu.arizona.biosemantics.rest.git.GlossaryTypeDAO;
import edu.arizona.biosemantics.rest.git.GlossaryVersionDAO;
...
public class IPlantGlossaryService {
	...
	private GlossaryDownload getLegacyGlossaryDownloadForIplant(
			edu.arizona.biosemantics.rest.beans.GlossaryDownload oldGlossaryDownload) {
		List<edu.arizona.biosemantics.rest.beans.TermCategory> oldTermCategories = oldGlossaryDownload.getTermCategories();
		List<edu.arizona.biosemantics.rest.beans.TermSynonym> oldTermSynonyms = oldGlossaryDownload.getTermSynonyms();
		List<TermCategory> termCategories = new LinkedList<TermCategory>();
		List<TermSynonym> termSynonyms = new LinkedList<TermSynonym>();
		for(edu.arizona.biosemantics.rest.beans.TermCategory oldTermCategory : oldTermCategories)
			termCategories.add(new TermCategory(oldTermCategory.getTerm(), oldTermCategory.getCategory(), oldTermCategory.isHasSyn()));
		for(edu.arizona.biosemantics.rest.beans.TermSynonym oldTermSynonym : oldTermSynonyms)
			termSynonyms.add(new TermSynonym(oldTermSynonym.getTerm(), oldTermSynonym.getSynonym()));
		
		GlossaryDownload result = new GlossaryDownload();
		result.setVersion(oldGlossaryDownload.getVersion());
